...
func TestRuntimeStopsTypingAndNotifiesWhenAppServerDisconnects(t *testing.T) {
	server := newFakeAppServer(t)
	defer server.Close()
	...
	time.Sleep(200 * time.Millisecond)
	if sender.chatActionCount("typing") != initialActions {
		t.Fatalf("typing chat actions kept increasing after app-server disconnect")
	}
}

func TestRuntimeStopsTypingAndNotifiesWhenTurnErrorArrives(t *testing.T) {
	server := newFakeAppServer(t)
	defer server.Close()
